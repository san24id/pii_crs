...
class RiskRegister extends APP_Controlleri {
	...
	public function index()
	{
		...
		<script src="assets/scripts/riski/risklist.js"></script>
		';
	...
	{
		$data = $this->loadDefaultAppConfig();
		$data['sidebarMenu'] = $this->getSidebarMenuStructure('riski/RiskRegister/undermaintenance');
...
//ubah 
	...
	{
		$data = $this->loadDefaultAppConfig();
		$data['sidebarMenu'] = $this->getSidebarMenuStructure('riski/RiskRegister/recover');
	...
	{
		$data = $this->loadDefaultAppConfig();
		$data['sidebarMenu'] = $this->getSidebarMenuStructure('riski/RiskRegister/recover_user');
		$data['indonya'] = base_url('index.php/riski/RiskRegister/recover');
		$data['engnya'] = base_url('index.php/risk/RiskRegister/recover');
		$data['pageLevelStyles'] = '
		<link rel="stylesheet" type="text/css" href="assets/global/plugins/datatables/plugins/bootstrap/dataTables.bootstrap.css"/>
		...
		<script src="assets/scripts/riski/risklist_recover_user.js"></script>
		';
	...
	public function RiskRegisterInputRac($mode = false, $username)
	{
		...
		<script src="assets/scripts/riski/riskinputrac.js"></script>
		';
	...
	public function loadRiskLibrary_primary_c($rid) 
	{
		if (!empty($rid) && is_numeric($rid)) {
			...
			//cek dulu dari library apa nggak
			$cek_from_library = $this->risk->cek_from_library($rid);
				$datax = $this->risk->getRiskByIdprimary_c($rid,$session_data['username']);
	...
	public function ChangeRequestOwned($risk_id)
	{
		...
		if (!empty($risk_id) && is_numeric($risk_id)) {
			...
			if ($res) {
				...
				<script src="assets/scripts/riski/cr_riskregister_own.js"></script>
				';
	...
	public function ChangeRequestAction($act_id)
	{
		...
		if ($action && !empty($act_id) && is_numeric($act_id)) {
			...
			if ($res) {
				...
				<script src="assets/scripts/riski/cr_riskregister_actplan.js"></script>
				';
