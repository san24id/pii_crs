			...
			<input type="hidden" value="<?=$action_plan['risk_id']?>" name="risk_id" placeholder="">
				<div class="form-body">
					...
					<div class="form-group" id="fgroup-date" <?=$g2?>>
						...
						<div class="col-md-9">
							<input type="text" class="form-control input-sm input-readview" name="revised_date" readonly value="<?=$action_plan['revised_date_v']?>">
						</div>
					</div>
					<?php }else{ ?>
...
<?php } ?>
					<?php if($_GET['kat']=='ap'){?>
								...
								<?php }else if($_GET['kat']=='acp'){?>
									<a href="<?php echo site_url('main/mainrac/actionplanexe_now'); ?>"><button type="button" class="btn yellow"><i class="fa fa-times"></i> Back</button></a>
								<?php } else if($status == 'under' || $_GET['kat']=='pri'){?>
