...
<div class="page-content-wrapper">
	<div class="page-content">
				<!-- BEGIN PAGE HEADER-->
		<h3 class="page-title">
		View Submission Risk Treatment Owner
		</h3>
		<div class="page-bar">
			<ul class="page-breadcrumb">
				...
				<li>
					<a target="_self" href="javascript:;">View Risk Treatment Form</a>
				</li>
			</ul>
		</div>
		<!-- END PAGE HEADER-->
						...
						<div class="form-body">
							...
							<table class="table table-condensed table-bordered table-hover">
								...
								<tr role="row" class="heading">
									...
									<th><small>Latest Status Of Execution</small></th>
								</tr>
								</thead>
								<tbody>
									...
									<tr>
										...
										<?php if($row['due_date_v'] == '00-00-0000'){
...
<?php 
  $sql_ap = "SELECT 
          ...
          AND c.division = a.division AND a.risk_id = '".$risk['risk_id']."' 
          ...
          AND a.`risk_id` = d.`risk_id` AND d.`periode_id` = (SELECT MAX(periode_id) FROM m_periode)
...
?>
					...
					<form role="form" id="input-form" class="form-horizontal">
						<div class="form-body">
							...
							<div class="form-group">
								<label class="col-md-3 control-label smaller cl-compact" title="fill this field with the description of set of factors that may affects or lead to the occurrence of risk event">Risk cause</label>
								<div class="col-md-9">
								<textarea class="form-control input-readview" readonly="true" rows="3" name="risk_cause" placeholder="" data-container="body" data-trigger="focus" data-placement="bottom" data-content=""></textarea>
								</div>
							</div>
							<div class="form-group">
								<label class="col-md-3 control-label smaller cl-compact" title="fill this field with the description of potential direct or indirect loss or cost to IIGF that could have been suffered from a risk event">Risk impact</label>
								<div class="col-md-9">
								<textarea class="form-control input-readview" readonly="true" rows="3" name="risk_impact" placeholder="" data-container="body" data-trigger="focus" data-placement="bottom" data-content=""></textarea>
								</div>
							</div>
							<div class="form-group">
								<label class="col-md-3 control-label smaller cl-compact" title="fill this field with grading as describe in “Impact Category” (e.g. insignificant, minor, moderate, major, and catastrophic) after consideration to existing control effectiveness">Impact Level </label>
									...
									<div class="col-md-9">
										...
										<input type="text" class="form-control input-sm input-readview" readonly="true" readonly="true" name="risk_impact_level_value" placeholder="">
									</div>
							...
							<div class="form-group">
								<label class="col-md-3 control-label smaller cl-compact" title="fill this field withthe grading criteria as described in “Likelihood of Risk Occurrence” table (e.g. very low, low, medium, high, and very high) after consideration to existing control effectiveness">Likelihood </label>
									...
									<div class="col-md-9">
										...
										<input type="text" class="form-control input-sm input-readview" readonly="true" readonly="true" name="risk_likelihood_value" placeholder="">
									</div>
							...
							<div class="form-group">
								...
								<label class="col-md-3 control-label smaller cl-compact" title="fill this field with the grading criteria as described in “Risk Level” matrix (e.g. insignificant, minor, moderate, major, and catastrophic) based on defined value in ‘Likelihood’ and ‘Impact Level’">Risk Level </label>
								<div class="col-md-9">
								<input type="text" class="form-control input-sm input-readview" readonly="true" readonly="true" name="risk_level" placeholder="">
								</div>
							</div>
							<hr/>
							<div class="row">
	...
	<?php 
		...
		$sql = "select suggested_risk_treatment from t_risk_own
				where risk_id = '".$risk['risk_id']."' and risk_input_by = '".$risk['risk_input_by']."'";
		...
		$sql2 = "select risk_owner, suggested_risk_treatment from t_risk
				where risk_id = '".$risk['risk_id']."' and risk_input_by = '".$risk['risk_input_by']."'";
		$query2 = $this->db->query($sql2);
		if ($query2->num_rows() > 0){
			$hasil2 = $query2->row();
			$risk_suggested = $hasil2->suggested_risk_treatment;
			$risk_own = $hasil2->risk_owner;
			
		}

	?>
		<input type="hidden" name="risk_suggested" value="<?php echo $risk_suggested; ?>"/>	
		<input type="hidden" name="risk_own" value="<?php echo $risk_own; ?>"/>	
		<input type="hidden" value="<?php echo $suggested; ?>" id="sgrt" />	
							<div class="row">
								<div class="col-md-6">
									<h4>Assigned Action Plan</h4>
								</div>
								<div class="col-md-6 clearfix" id="btnaction">
								</div>
							</div>
							...
							<div class="table-scrollable">
								<table id="action_plan_table" class="table table-condensed table-bordered table-hover">
									...
									<tr role="row" class="heading">
										...
										<th><small>Due Date</small></th>
										<th><small>Action Plan Owner</small></th>
										<th><small>Latest Status <br>Of Execution</small></th>
						...
						<div class="form-actions right">
							<button type="button" class="btn yellow" onclick="window.history.go(-1); return false;"><i class="fa fa-times"></i> Back</button>
						</div>
					</form>
				</div>	
			</div>
