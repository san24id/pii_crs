		...
		<div class="col-md-12">
			...
			<div class="form-body">
				...
				<div class="form-group">
					...
					<?php }else{ ?>
						<input data-toggle="toggle" type="checkbox" checked="checked" data-size="small" data-width="140" data-onstyle="danger" data-on = "Mandatory" data-off = "Non Mandatory" disabled="disabled" />
						<input type="hidden" class="form-control input-sm input-readview" readonly="true" name="mandatory" placeholder="" value="<?=$kri['mandatory']?>">
					<?php } ?>
					</div>
				</div>
				...
				<div class="form-group">
				<label class="col-md-2 control-label" title="">Response</label>
				<div class="col-md-6">		
				<textarea class="form-control input-sm popovers"  data-container="body" data-placement="bottom" rows="3" id="owner_report" name="owner_report"></textarea>
				</div>
				</div>	
				...
				<div class="col-md-6">		
				<textarea class="form-control input-sm popovers"  data-container="body" data-placement="bottom" rows="3" name="supporting_evidence" id="supporting_evidence" > </textarea>
				</div>
				</div>
			...
			<div class="form-actions right">
				...
				<button id="kri-button-save-modif_verify" type="button" class="btn blue"><i class="fa fa-circle-o"></i> Save</button>
				<button id="kri-button-delete-v" type="button" class="btn red"><i class="fa fa-times"></i>Delete</button>
...
<div id="modal-category" class="modal fade" tabindex="-1" data-width="760" data-backdrop="static" data-keyboard="false">
	...
	<div class="modal-body" style="display: none;">
			<form id="modal-category-form_no" role="form" class="form-horizontal">
						...
						<div class="col-md-9">
							<div class="input-group">
								 <input type="hidden" name="risk_likelihood_id" id = "risk_likelihood_id" value="<?php echo $sql2_run->risk_likelihood_key; ?>"/>
									<input type="text" class="form-control input-sm" readonly="true" name="risk_likelihood_key_after_kri" id = "risk_likelihood_key_after_mitigation" value="<?php $i = strtolower($sql2_run->risk_likelihood_key); echo ucwords($i); ?>" placeholder="">
								<span class="input-group-btn">
		...
		<table class="table table-condensed table-bordered table-hover">
			...
			<?php $cnt = 0;
				foreach($likelihood as $row) { 
						if($sql2_run->risk_likelihood_key_after_kri == NULL || $sql2_run->risk_likelihood_key_after_kri == ''){
							$i = $sql2_run->risk_likelihood_key;
						}else{
							$i = $sql2_run->risk_likelihood_key_after_kri;
						}
					?>

				<tr>
					<td><label><input type="radio" name="risk_likelihood" id="likelihood_<?=$row['l_id']?>" value="<?=$row['l_key']?>|<?=$row['l_title']?>" <?php echo ($row['l_key'] == $i) ? 'checked' : ''?>></label></td>
					<td><?=$row['l_title']?></td>
					<td><?=$row['l_desc']?></td>
				</tr>
			<?php $cnt++; } ?>
