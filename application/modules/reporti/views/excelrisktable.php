...
header("Content-Disposition: attachment; filename=Risk_Table". date('dmY_His').".xls" );
header("Expires: 0");
...
$worksheet1->write_string(0,4,'Risk Table',$title);  // Set Nama kolom
...
$worksheet1->write_string(2,4,'Impact Level',$header);  // Set Nama kolom
$worksheet1->set_column(0,4,50); // Set lebar kolom
$worksheet1->write_string(2,5,'Likelihood Level',$header);  // Set Nama kolom
$worksheet1->set_column(0,4,50); // Set lebar kolom
$worksheet1->write_string(2,6,'Current Periode',$header);  // Set Nama kolom
$worksheet1->set_column(0,4,50); // Set lebar kolom
$worksheet1->write_string(2,7,'Previous Periode',$header);  // Set Nama kolom
$worksheet1->set_column(0,4,50); // Set lebar kolom
...
if($rows>0){
	if($i < $rows){
		foreach ($data as $key) {
		    ...
		    $worksheet1->write_string($row,6,  $key->curr ,$content);
		    $worksheet1->write_string($row,7,  $key->prev ,$content);
		    $row++;
		    $i++;
		}	
