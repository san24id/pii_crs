...
defined('BASEPATH') OR exit('No direct script access allowed');
	...
	class Risk extends APP_Controller {
		...
		public function listofaprisk(){
			$this->load->model('report/risk_model','risk',true);

			$data = $this->loadDefaultAppConfig();
			$data['indonya'] = base_url('index.php/reporti/risk/listofaprisk');
			$data['engnya'] = base_url('index.php/report/risk/listofaprisk');		
			$data['sidebarMenu'] = $this->getSidebarMenuStructure('report/risk/listofaprisk');
			$data['periode'] = $this->risk->getAllPeriode();
			$data['pageLevelStyles'] = '
			<link rel="stylesheet" type="text/css" href="assets/global/plugins/datatables/plugins/bootstrap/dataTables.bootstrap.css"/>
			';
			...
			$this->load->view('main/header', $data);
			$this->load->view('report_listofAPErisk', $data);
			$this->load->view('footer', $data);
		}	
		
		function listofaprisk_excel(){
			
			$this->load->model('report/risk_model','risk',true);
			$this->load->library('parser');
			  
			$data['datanya'] = $this->risk->listofaprisk($this->input->post());
			...
			$stringData = $this->parser->parse('listofaperisk_table', $data, true);
			...
			header('Content-type: application/ms-excel');
			header("Expires: 0");
			header('Content-Disposition: attachment; filename=listAPEonByRisk.xls');
			header("Content-Description: File Transfer");
		...
		function listofaprisk_pdf(){
			...
			$this->load->view('listofaperisk_table',$data);
			// Get output html
			$html = $this->output->get_output();
			...
			$this->dompdf->set_paper($paper_size, $orientation);
			$this->dompdf->render();			
			$this->dompdf->stream("listAPEonByRisk.pdf");
		...
		public function score2(){
			...
			$data['sidebarMenu'] = $this->getSidebarMenuStructure('report/risk/score2');
			$data['periode'] = $this->risk->getAllPeriode();
			$data['pageLevelStyles'] = '
			<link rel="stylesheet" type="text/css" href="assets/global/plugins/datatables/plugins/bootstrap/dataTables.bootstrap.css"/>
			';
		...
		function riskmapdivisi_excel(){
			...
			$data['datanya'] = $this->risk->riskmapdivisireport($div, $a, $b);
			
			$data['cekperiode'] = $this->risk->cekperiode($this->input->post('periode'));
			
			$data['total_data'] = count($data['datanya']);
			 
			$stringData = $this->parser->parse('riskmapdivreport_table', $data, true);
			...
			header('Content-type: application/ms-excel');
			header("Expires: 0");
			header('Content-Disposition: attachment; filename='.$nfile.'.xls');
			header("Content-Description: File Transfer");
		...
		function riskmapdivisi_pdf(){
			...
			$this->load->view('riskmapdivreport_table',$data);
			// Get output html
			$html = $this->output->get_output();
			...
			$this->dompdf->set_paper($paper_size, $orientation);
			$this->dompdf->render();			
			$this->dompdf->stream("riskmapcatavlow.pdf");
