...
class APP_Controlleri extends MX_Controller {
	...
	private function __getRecursiveMenu($par_id) {
		$sql = "select * from m_menu_i where menu_parent = ? order by menu_order";
	...
	public function getSidebarMenuStructure($active_module = null) {
		...
		if ($session_data['role_id'] == 1) { // admin get all menu structure
			$sql = "select * from m_menu_i where menu_parent = 0 and menu_id > 22 order by menu_order";
