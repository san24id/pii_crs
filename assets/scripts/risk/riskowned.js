...
gridControlobjective.init({
    ...
    dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options 
        ...
        "columnDefs": [ {
            ...
            "render": function ( data, type, full, meta ) {
                var ret = '<div class="btn-group">'+
                '<button type="button" class="btn btn-default btn-xs" onclick="javascript: RiskOwned.selectControlLibraryobjective('+full.id+')"><i class="fa fa-check-circle font-blue"> Select </i></button>'+
                '</div>';
                return ret;
            }
        } ],
    ...
    return {
        ...
        init: function(mode) {
            ...
            */
            ...
            $('#risk-button-cancel').on('click', function() {
                var mod = MainApp.viewGlobalModal('confirm', 'Are You sure want to cancel your Risk Treatment ? You will loose your unsaved data.');
                mod.find('button.btn-primary').off('click');
                mod.find('button.btn-primary').one('click', function(){
                    location.href=site_url+'/main/mainpic#tab_owned';
                });
            });
        ...
        controlAddRowobjective: function(nnode) {
            ...
            $('#objective_table > tbody:last-child').append('<tr id = "'+me.dataControlCounterobjective+'">'+
                ...
                '<div class="btn-group">'+
                    ...
                    '<button type="button" class="btn btn-default btn-xs" onclick="RiskOwned.controlTableDeleteobjective(this, '+me.dataControlCounterobjective+')"><i class="fa fa-trash-o font-red"> Delete </i></button>'+
                '</div>'+
                '</td>'+
            '</tr>');
        ...
        submitRiskData: function(submitMode) {
            ...
            if (submitMode == 'draft') {
                ...
                var vtext = 'Are You sure want to Save this risk as Draft ?';
            } else {
                var url = site_url+'/main/mainpic/treatmentSubmit';
                var vtext = 'Are You sure want to Submit this Risk Treatment ?';
            }
