    ...
    return {
        ...
        init: function(mode) {
            ...
            $('#risk-button-draft').on('click', function() {
                ...
                mod.find('button.btn-primary').one('click', function(){
                    me.submitRiskEditData();
                });
            });

            $('#risk-button-transfer').on('click', function() {
                var mod = MainApp.viewGlobalModal('confirm', 'Are You sure you want to Transfer Risk to Library ?');
                mod.find('button.btn-primary').one('click', function(){
                    me.submitRiskTrasferData();
                });
            });
        ...
        submitRiskData: function() {
            ...
            if (fvalid) {
                ...
                var url = site_url+'/risk/RiskRegister/insertRiskAgregateNew';
                $.post(
                    url,
                    ...
                    function( data ) {
                        ...
                        if(data.success) {
                            ...
                            mod.find('button.btn-ok-success').one('click', function(){
                                ...
                                location.href=site_url+'/main/mainrac/aggregatrisknew/'+pid;
                            });
        ...
        submitRiskEditData: function() {
            ...
            if (fvalid) {
                ...
                Metronic.blockUI({ boxed: true });
                //var url = site_url+'/risk/RiskRegister/insertRiskData/'+g_username_user;
                var url = site_url+'/risk/RiskRegister/saveRiskDataAggNew';
                $.post(
                    url,
                    $( "#input-form" ).serialize()+ '&' + $.param(impact_param)+ '&' + $.param(actplan_param)+ '&' + $.param(control_param)+ '&' + $.param(objective_param),
                    function( data ) {
        ...
        submitRiskTrasferData: function() {
            var form1 = $('#input-form').validate();
            var fvalid = form1.form();
            if (fvalid) {
                var me = this;
                ...
                Metronic.blockUI({ boxed: true });
                //var url = site_url+'/risk/RiskRegister/insertRiskData/'+g_username_user;
                var url = site_url+'/risk/RiskRegister/transferRiskAgregateNew';
                $.post(
                    url,
                    $( "#input-form" ).serialize()+ '&' + $.param(impact_param)+ '&' + $.param(actplan_param)+ '&' + $.param(control_param)+ '&' + $.param(objective_param),
                    function( data ) {
