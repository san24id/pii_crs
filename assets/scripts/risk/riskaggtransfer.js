var RiskTransfer = function() {
	return {
		init: function() {
        	var me = this;
        	
        	me.loadRiskPrimary(g_id_ag);
        	
        	$('#div-portlet-page-caption').html('Verify Risk Form');
        	
        	$('#primary-risk-button-verify').on('click', function() {
        		var mod = MainApp.viewGlobalModal('confirm', 'Are You sure you want Tansfer Risk Aggregate to RIsk Libarary this risk ?');
        ...
        loadRiskPrimary: function(rid) {
        	var me = RiskInputTransfer;
        	
        	$('#modal-library').modal('hide');
        	Metronic.blockUI({ boxed: true });
        	$.getJSON( site_url+"/risk/RiskRegister/loadRiskAggregate/"+rid, function( data_risk ) {
        		Metronic.unblockUI();
        ...
        submitRiskData: function(submitMode) {
        	...
        	if (fvalid) {
            	...
            	if (submitMode == 'verify') {
            		var url = site_url+'/main/mainrac/transferRiskDataAgg';
            		...
            		function( data ) {
            			...
            			if(data.success) {
            				...
            				mod.find('button.btn-ok-success').one('click', function(){
            					...
            					if (submitMode == 'verify') {
            						location.href=site_url+'/main/mainrac/riskTransferAggForm/'+pid+'/'+g_id_ag+'/'+g_risk_id;
            					}else if (submitMode == 'save-under') {
        ...
        saveImpactEdit: function(submitMode) {
            ...
            if (fvalid) {
                ...
                $.post(
                    url,
                    $( "#primary-input-form" ).serialize()+ '&' + $.param(impact_param),
                    function( data ) {
                        Metronic.unblockUI();
                        if(data.success) {
                            var mod = MainApp.viewGlobalModal('success', 'Success Updating your Risk');
