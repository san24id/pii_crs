...
var gridDivision = new Datatable();
gridDivision.init({
    src: $("#library_divisi_table"),
    onSuccess: function (grid) {
        // execute some code after table records loaded
    },
    onError: function (grid) {
    ...
    dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options 
        ...
        "lengthMenu": [[10], [10]],
        "ajax": {
            "url": site_url+"/risk/RiskRegister/getDivision" // ajax source
        },
        "columnDefs": [ {
            "targets": 0,
            "data": "id_div",
            "render": function ( data, type, full, meta ) {
                var ret = '<div class="btn-group">'+
                '<button type="button" class="btn btn-default btn-xs" onclick="javascript: RiskInput.selectControlLibraryDivisi('+full.id_div+')"><i class="fa fa-check-circle font-blue"> Select </i></button>'+
                '</div>';
                return ret;
            }
        }],
        "columns": [
            { "data": "id_div", "orderable": false },
            { "data": "division_name"}
       ],
        "order": [
            [1, "asc"]
...
var handleValidationDivisi = function() {
    var form1 = $('#input-form-control-divisi');
...
var RiskInput = function() {
    return {
        ...
        init: function(mode) {
            ...
            $('#modal-control-filter-submit-divisi').on('click', function() {
                me.filterControldivisi();
            });
            ...
            $('#input-control-add-divisi').on('click', function() {
                var form1 = $('#input-form-control-divisi').validate();
                var fvalid = form1.form();
                 
                if (fvalid) {
                    var xcid = $('#input-form-control-divisi input[name=division_id]').val();
                    var xcobjid = $('#input-form-control-divisi input[name=division_name]').val();
                  
                    
                    var tr_idob = $('#form-control-revid-divisi').val();
                    ...
                    var nnode = {
                        'tr_idob' : tr_idob,
                        'division_id' : xcid,
                        'division_name' : xcobjid,
                        
                    };
                    
                    me.controlAddRowdivisi(nnode);
                    
                    $('#form-control-divisi').modal('hide');
                }
            });
            ...
            */
            ...
            $('#button-form-control-open-divisi').on('click', function () {
                document.getElementById("input-form-control-divisi").reset();
               // $('#input-form-control-divisi')[0].reset();
                $('#form-control-revid-divisi').val("");
                $('#input-form-control-divisi text[name=division_id]').attr('readonly', false);
                $('#input-form-control-divisi text[name=division_name]').attr('readonly', false);
            });
            ...
            $('#risk-button-update').on('click', function() {
                me.updateRiskData();
            });
        ...
        controlTableDeletedivisi: function(xrow, dataId) {
            //console.log(dataId);
            var i=xrow.parentNode.parentNode.parentNode.rowIndex;
            document.getElementById('divisi_table').deleteRow(i);
            this.controlDeletedivisi(dataId);
        },
        ...
        controlResetdivisi: function() {
            this.dataControldivisi = {};
            this.dataControlCounterdivisi = 0;
            $("#divisi_table > tbody").html("");
        },
        controlAdd: function(data, dcounter) {
            this.dataControl[dcounter] = data;
        },
        controlAdddivisi: function(data, dcounter) {
            this.dataControldivisi[dcounter] = data;
        },
        controlDelete: function(id) {
            delete this.dataControl[id];
        },
        controlDeletedivisi: function(id) {
            delete this.dataControldivisi[id];
        },
        ...
        controlAddRowdivisi: function(nnode) {
            var me = this;
            
            var lastidrand = $('#form-control-revid-divisi').val();
            ...
            me.dataControlCounterdivisi++; 
            
            var idrand = Math.floor((Math.random() * 1000000) + 1); 

            $('#divisi_table > tbody:last-child').append('<tr id = "'+me.dataControlCounterdivisi+'">'+
                '<td><input type = "hidden" id = "division_id'+me.dataControlCounterdivisi+'" value = "'+nnode.division_id+'">'+
                '<input type = "hidden" id = "division_name'+me.dataControlCounterdivisi+'" value = "'+nnode.division_name+'">'+nnode.division_name+'</td>'+
                '<td>'+
                '<div class="btn-group">'+
                    '<button type="button" class="btn btn-default btn-xs" onclick="RiskInput.controlTableDeletedivisi(this, '+me.dataControlCounterdivisi+')"><i class="fa fa-trash-o font-red"> Delete </i></button>'+
                '</div>'+
                '</td>'+
            '</tr>');
            this.controlDeletedivisi(nnode.tr_idob); 
            me.controlAdddivisi(nnode, me.dataControlCounterdivisi);
        },
        ...
        */
        ...
        selectControlLibraryDivisi: function(rid) {
            var me = this;
            $('#input-form-control-divisi text[name=division_id]').attr('readonly', false);
            $('#input-form-control-divisi text[name=division_id]').attr('readonly', false);
            ...
            Metronic.blockUI({ boxed: true });
            $.getJSON( site_url+"/risk/RiskRegister/loadControlLibraryDivisi/"+rid, function( data_risk ) {
                Metronic.unblockUI();
                
                me.populateRisk($('#input-form-control-divisi'), data_risk);
            });
        },
        ...
        filterControldivisi: function() {
            var fval = $('#modal-divisi div.inputs input[name=filter_search]')[0].value;
            gridDivision.clearAjaxParams();
            gridDivision.setAjaxParam("filter_library", fval);
            gridDivision.getDataTable().ajax.reload();   
        },
        ...
        submitRiskData: function() {
            ...
            if (fvalid) {
                ...
                var divisi_param = {};
                var cnt = 0;
                $.each(me.dataControldivisi, function(key, value) { 
                    divisi_param['divisi['+cnt+'][division_id]'] = value.division_id;
                    divisi_param['divisi['+cnt+'][division_name]'] = value.division_name;
                    cnt++;
                });
                
                if (cnt < 1) {
                    MainApp.viewGlobalModal('error', 'Please Input at least One Divisi for this Risk');
                    return false;
                }
                ...
                var url = site_url+'/risk/RiskRegister/insertLossEvent';
                $.post(
                    url,
                    $( "#input-form" ).serialize()+ '&' + $.param(divisi_param),
                    function( data ) {
                        Metronic.unblockUI();
                        if(data.success) {
                            ...
                            mod.find('button.btn-ok-success').one('click', function(){
                                ...
                                location.href=site_url+'/main/mainrac/lossevent';
                            });
        ...
        updateRiskData: function() {
            ...
            if (fvalid) {
                ...
                Metronic.blockUI({ boxed: true });
                //var url = site_url+'/risk/RiskRegister/insertRiskData/'+g_username_user;
                var url = site_url+'/risk/RiskRegister/updateLossEvent';
                $.post(
                    url,
                    $( "#input-form" ).serialize()+ '&' + $.param(divisi_param),
                    function( data ) {
