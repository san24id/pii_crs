...
gridRiskList.init({
    ...
    dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options 
        ...
        "stateSave" : true,    
        "ajax": {
            "url": site_url+"/main/mainrac/getAllLossEvent" // ajax source
        },
    "columnDefs": [{
            "targets": 7,
            "data": "id_event",
            "render": function ( data, type, full, meta ) {
                var img = '<div class="btn-group">'+
                            '<a href="'+site_url+'/risk/RiskRegister/EditLossEvent/'+full.id_event+'"><button type="button" class="btn btn-default btn-xs button-grid-update"><i class="fa fa-edit font-blue"> Update </i></button></a>'+
                             '<a href="'+site_url+'/risk/RiskRegister/ViewLossEvent/'+full.id_event+'"><button type="button" class="btn btn-default btn-xs button-grid-update"><i class="fa fa-eye font-green"> View </i></button></a>'+
...
var Dashboard = function() {
    return {
        ...
        deleteLossEvent: function(data) {
                var mod = MainApp.viewGlobalModal('warning', 'Are you sure want to <b>Delete</b> risk : <b>'+data.nama_proyek+'</b> ?');
                mod.find('button.btn-danger').off('click');
                mod.find('button.btn-danger').one('click', function(){
                    mod.modal('hide');
                    var eparam = {
                    ...
                    var url = site_url+'/main/mainrac/deleteLossEvent';          
                    Metronic.blockUI({ boxed: true });
                    $.post(
                        url,
                        $.param(eparam),
