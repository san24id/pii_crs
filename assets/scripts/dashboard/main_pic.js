                ...
                if (data >= 2 && data < 6) {
                    if (g_submit_mode == true){
                        if(full.risk_existing_control == 'request'){
                            ...
                            if(full.risk_library_id == null){
                                tt = '<div class="btn-group">'+
                                    '<button type="button" class="btn btn-sky blue btn-xs button-grid-edit" onclick="location.href=\''+site_url+'/risk/RiskRegister/ChangeRequestInput/'+full.risk_id+'/'+full.risk_input_by+'\'"><i class="fa fa-pencil"> Change Request  </i></button>'+ 
                                '</div>';
                            }else{
                               tt = '<div class="btn-group">'+
                                    '<button type="button" class="btn btn-sky blue btn-xs button-grid-edit" onclick="location.href=\''+site_url+'/risk/RiskRegister/ChangeRequestInputchange/'+full.risk_id+'/'+full.risk_input_by+'\'"><i class="fa fa-pencil"> Change Request  </i></button>'+ 
                                '</div>'; 
                            }
                        }
                    }else{
...
var Dashboard = function() {
    return {
        ...
        //end 
            ...
            $('#r_level-treatment').change(function(){
                var risk_level = $('#r_level-treatment').val();
                $('input[id="tab_treatment_list-filterValue"]').attr('value', risk_level);
                if(risk_level == "-"){
                    $('#tab_treatment_list-filterValue').val('');
                }
            });
        ...
        //end 
            ...
            $('#sel_risk_sub_category').hide();
            $('#sel_risk_2nd_sub_category').hide();
            $('#button-filter-category').hide();
            $('#sel_old_risk_sub_category').hide();
            $('#sel_old_risk_2nd_sub_category').hide();
            $('#button-old-filter-category').hide();

            $('#sel_risk_category').change(function(){
                var category = $('#sel_risk_category').val();
                if(category == 'choose'){
                    $('#sel_risk_sub_category').hide();
                    $('#sel_risk_2nd_sub_category').hide();
                    $('#button-filter-category').hide();
                }else{
                    $('#sel_risk_sub_category').show();
                    $('#sel_risk_2nd_sub_category').show();
                    $('#button-filter-category').show();
                }
            });

            $('#sel_old_risk_category').change(function(){
                var category = $('#sel_old_risk_category').val();
                if(category == 'choose'){
                    $('#sel_old_risk_sub_category').hide();
                    $('#sel_old_risk_2nd_sub_category').hide();
                    $('#button-old-filter-category').hide();
                }else{
                    $('#sel_old_risk_sub_category').show();
                    $('#sel_old_risk_2nd_sub_category').show();
                    $('#button-old-filter-category').show();
                }
            });
...
$('#submit-risk_owner').click(function(e) {
                    e.preventDefault();
                    var l = Ladda.create(this);
                    l.start();
                    
                        var url = site_url+'/main/mainpic/submitstatement_risk_owner';
                    ...
                    $.post(
                        ...
                        function( data ) {
                            ...
                            if(data.success) {
                                ...
                                $('#modal-riskowner').modal('hide');
                                $('#submit-risk_owner').hide();
                                $('#ar1').show();
  ...
  $('#submit-action_plan').click(function(e) {
                    e.preventDefault();
                    var l = Ladda.create(this);
                    l.start();
                    
                        var url = site_url+'/main/mainpic/submitstatement_action_plan';
                    ...
                    $.post(
                        ...
                        function( data ) {
                            ...
                            if(data.success) {
                                ...
                                $('#modal-action_plan').modal('hide');
                                 $('#submit-action_plan').hide();
                                 $('#ar2').show();
