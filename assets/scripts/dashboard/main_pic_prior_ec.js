...
grid_exec.init({
    ...
    dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options 
        ...
        "ajax": {
            "url": site_url+"/main/mainpic/getActionPlanExec_prior" // ajax source
        },
        "columnDefs": [ {
            "targets": 0,
            ...
            "render": function ( data, type, full, meta ) {
                ...
                var dat = '';
               if  (ret == '' || ret == null) {
                    return '<a target="_self" href="'+site_url+'/main/mainpic/viewOwnedActionPlanPrior/'+full.id_p+'/'+full.action_plan_status+'?kat=acp">'+data+'</a>';
                } else {
                    return '<a target="_self" href="'+site_url+'/main/mainpic/viewOwnedActionPlanPrior/'+full.id_p+'/'+full.action_plan_status+'?kat=acp">'+data+'</a>';
                }
                return dat;
           ...
           "render": function ( data, type, full, meta ) {
            ...
            "render": function ( data, type, full, meta ) {
                ...
                if(full.action_plan_status != 7 && full.action_plan_status != 6){
                    var search_text = '<button type="button" class="btn blue btn-xs button-grid-search"><i class="fa fa-search"> Submit </i></button>';
                    //var submit_text = '<button type="button" class="btn blue btn-xs button-grid-submit"><i class="fa fa-check-circle"></i> Submit</button>';
                }else{
                    var search_text = '<button type="button" class="btn blue btn-xs button-grid-search"><i class="fa fa-search"> View </i></button>';
                    //var submit_text = '<button type="button" class="btn blue btn-xs button-grid-submit"><i class="fa fa-check-circle"></i> Submit</button>';
                }
            ...
            "render": function ( data, type, full, meta ) {
                ...
                if (data == 6) {
                     ...
                     tt = '<div class="btn-group">'+
                            '<button type="button" class="btn blue btn-xs button-grid-edit" onclick="location.href=\''+site_url+'/main/mainpic/ChangeRequestActionExePrior/'+full.id_p+'/'+full.action_plan_status+'\'"><i class="fa fa-pencil"> Change Request </i></button>'+
                        '</div>';
                    }
                }
                return tt;
    ...
    return {
        ...
        init: function() {
            ...
            $("#datatable_action_exec").on('click', 'button.button-grid-submit', function(e) { 
                ...
                var url = site_url+'/main/mainpic/execSubmitPrior';
                    ...
                    $.post(
                        ...
                        function( data ) {
                            ...
                            if(data.success) {
                                ...
                                window.location.href = site_url+'/main/mainpic/actionplanexe_prior';
...
//simpan yang on going          
            $('#exec-button-save').on('click', function() {
                var me = this;
                var url = site_url+'/main/mainpic/execSaveDraftPrior';
                var valid = false;
        ...
        $('#exec-button-submit').on('click', function() {
                var me = this;
                var url = site_url+'/main/mainpic/ap_execSubmitPrior';
                var valid = false;
...
$('#ar3').hide();
            $('#submit-action_plan_exe').click(function(e) {
                    ...
                    l.start();
                        ...
                        var url = site_url+'/main/mainpic/submitstatementprior_action_plan_exe';
        ...
        ownedAssignPic: function(risk_id, pic, mode) {
            Metronic.blockUI({ boxed: true });
            var url = site_url+'/main/mainpic/assignPicPrior';
            $.post(
                url,
