...
var handleValidation = function() {
	...
	form1.validate({
	    ...
	    invalidHandler: function (event, validator) { //display error alert on form submit              
	        MainApp.viewGlobalModal('error', 'Mohon isi Kolom yang Madatory');
	    },
	...
	return {
        ...
        init: function(mode) {
            ...
            */
        	...
        	$('#risk-button-cancel').on('click', function() {
        		var mod = MainApp.viewGlobalModal('confirm', 'Apakah Anda ingin membatalkan Risk Register Anda? Perubahan yang Anda buat mungkin tidak disimpan.');
        ...
        controlAddRow: function(nnode) {
        	...
        	$('#control_table > tbody:last-child').append('<tr id = "tr_c'+me.dataControlCounter+'">'+
        		'<td><input type = "hidden" id = "existing_control_id'+me.dataControlCounter+'" value = "'+nnode.existing_control_id+'">EC.'+nnode.existing_control_id+'</td>'+
        ...
        selectControlLibrary: function(rid) {
        	...
        	$.getJSON( site_url+"/risk/RiskRegister/loadControlLibrary/"+rid, function( data_risk ) {
        		...
        		data_risk['existing_control_id'] = data_risk['ec_id'];
        	...
        	if (fvalid) {
	        	...
	        	$.post(
	        		...
	        		function( data ) {
	        			Metronic.unblockUI();
	        			if(data.success) {
	        				var mod = MainApp.viewGlobalModal('success', 'Sukses Memasukkan Resiko Anda');
