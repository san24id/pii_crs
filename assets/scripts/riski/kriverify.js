	...
	return {
        ...
        init: function() {
			...
			$('#modal-impactlevel-form-submit-no').on('click', function() {
        		...
        		me.submitRiskData_no();
                        });				 
        	});

            $('#kri-button-delete_verify_yes').on('click', function() {
                        var mod = MainApp.viewGlobalModal('confirm', 'Apakah Anda yakin ingin menghapus KRI Mitigation dengan status Siap ?');
                        mod.find('button.btn-primary').one('click', function(){
        	...
        	$('#kri-button-cancel').on('click', function() {
        		var mod = MainApp.viewGlobalModal('confirm', 'Apakah Anda yakin ingin membatalkan Verifikasi KRI Anda? Anda akan kehilangan data yang belum disimpan.');
        		mod.find('button.btn-primary').one('click', function(){
        	...
        	if (fvalid) {
        		...
        		$.post(
        			url,
        			{
						...
						'risk_impact_level_after_kri' : $('#risk_impact_level_after_mitigation').val().toUpperCase(),
						'risk_likelihood_key_after_kri' : $('#risk_likelihood_key_after_mitigation').val().toUpperCase(),
						'risk_level_after_kri' : $('#risk_level_after_mitigation').val().toUpperCase()
        			...
        			function( data ) {
        				...
        				Metronic.unblockUI();
        				if(data.success) {
        					var mod = MainApp.viewGlobalModal('success', 'Sukses Memperbarui KRI');
                ...
                if (fvalid) {
                        ...
                        $.post(
                                ...
                                function( data ) {
                                        ...
                                        Metronic.unblockUI();
                                        if(data.success) {
                                                var mod = MainApp.viewGlobalModal('warning', 'Sukses Menghapus KRI');
                                                mod.find('button.btn-ok-success').one('click', function(){
                                                        location.href=site_url+'/main/mainrac#tab_kri_list';
                                                });
