...
var ChangeRequest = function() {
	return {
		...
		init: function() {
            ...
            */
        	...
        	$("#action_plan_table").on('click', 'button.button-grid-delete', function(e) {
        		...
        		var mod = MainApp.viewGlobalModal('confirm', 'Hapus Action Plan ?');
        ...
        loadRiskPrimary: function(rid) {
        	...
        	Metronic.blockUI({ boxed: true });
        	$.getJSON( site_url+"/risk/RiskRegister/loadRiskLibrary_primary_c/"+rid, function( data_risk ) {
        		Metronic.unblockUI();
            	...
            	$.post(
            		...
            		function( data ) {
            			...
            			if(data.success) {
                            ...
                            var mod = MainApp.viewGlobalModal('warning-maintenance', 'Risiko ini telah di edit oleh Tim RAC, Mohon Coba Lagi');
                            mod.find('button.btn-ok-success').one('click', function(){
                                location.href=site_url+'/main';
                            });
                        }else {
