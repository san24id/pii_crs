    ...
    return {
            ...
            $('#select-treshold-type').on('change', function() {
                if($('#select-treshold-type').val() == ''){ 
                    if (me.dataTresholdCounter > 0) {
                        var mod = MainApp.viewGlobalModal('confirm', 'Apakah Anda yakin ingin mengubah tipe treshold? Ini akan menghapus semua pengaturan treshold sebelumnya');
         ...
         /*   $('#is_numeric_treshold').on('click', function() {
          ...
          $('#button-kri-open-treshold2').on('click', function() {
            ...
            $('#kri-button-delete').on('click', function() {
                var mod = MainApp.viewGlobalModal('confirm', 'Apakah Anda yakin ingin menghapus Entri KRI Anda?');
                mod.find('button.btn-primary').one('click', function(){
                    me.deleteKRI_modif();
                });
            });
            ...
            $('#modal-impactlevel-form-submit-no').on('click', function() {
                        var mod = MainApp.viewGlobalModal('confirm', 'Apakah Anda yakin ingin Verifikasi KRI ini?');
                        mod.find('button.btn-primary').one('click', function(){
            ...
            $('#kri-button-delete-v').on('click', function() {
                var mod = MainApp.viewGlobalModal('confirm', 'Apakah Anda yakin ingin menghapus KRI dengan Mitigasi Status?');
                mod.find('button.btn-primary').one('click', function(){
        ...
        saveRiskData: function() {
            if ($('#risk_library_id').val() == '') {
                MainApp.viewGlobalModal('error', 'Mohon Pilih Risiko');
                return false;
            }
            ...
            if (fvalid) {
               ...
               if($('#cek-mitigation').checked) {
                if (cnt < 1) {
                    MainApp.viewGlobalModal('error', 'Silahkan masukan di Treshold Configuration');
                    return false;
                  }
                }
                ...
                $.post(
                    ...
                    function( data ) {
                        Metronic.unblockUI();
                        if(data.success) {
                            var mod = MainApp.viewGlobalModal('success', 'Sukses Memasukkan Kri anda');
               ...
               if($('#cek-mitigation').checked) {
                ...
                $.post(
                    ...
                    function( data ) {
                        Metronic.unblockUI();
                        if(data.success) {
                            var mod = MainApp.viewGlobalModal('success', 'Sukses Menyimpan Kri anda');
                            mod.find('button.btn-ok-success').one('click', function(){
            ...
            if (fvalid) {
                ...
                $.post(
                    ...
                    function( data ) {
                        Metronic.unblockUI();
                        if(data.success) {
                            var mod = MainApp.viewGlobalModal('success', 'Sukses Menghapus Kri anda');
                            mod.find('button.btn-ok-success').one('click', function(){
                                location.href=site_url+'/main/mainrac#tab_kri_list';
                            });
